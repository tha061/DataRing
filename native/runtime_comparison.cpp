...
/*	
	while (itr <= num_query)
    {
		...
		t2 = high_resolution_clock::now();
		trackTaskPerformance(time_track_list, "Gen Query_all_rows (ms)", t1, t2);
...
*/
	...
	// {
		...
		t2 = high_resolution_clock::now();
		trackTaskPerformance(time_track_list, "Matching (ms)", t1, t2);
		

		t1 = high_resolution_clock::now();
		server1.generateTest_Target_Attr_opt(pre_enc_stack);
		t2 = high_resolution_clock::now();
		trackTaskPerformance(time_track_list, "Gen Test estimate (ms)", t1, t2);
		...
		t2 = high_resolution_clock::now();
		trackTaskPerformance(time_track_list, "verify ans Test estimate (ms)", t1, t2);	
	// }
...
int runtime_comparison(int argc, char **argv)
{
    ...
    // ==== NORMAL QUERY targeting all records to compare with Drynx paper (compute average heart rate over 10 DPs, total dataset is 600K) ============//
	...
	if (argc > 1)
	{
		...
		{
			fout.open("./results/runtime_compare_query_500K_5_attr.csv", ios::out | ios::trunc);
		...
		{
			fout.open("./results/runtime_compare_query_500K_5_attr.csv", ios::out | ios::app);
		}
