...
int query_evaluation_with_cheating_detection_random_query_ans(int argc, char **argv)
{
	...
	float epsilon = noise_budget_party/num_query;  
	...
	part_A.maxNoise_q = getLaplaceNoiseRange(sensitivity, epsilon, percentile_noise);
	part_A.epsilon_q = epsilon; 
	part_A.minNoise_q = -part_A.maxNoise_q;
	part_A.maxNoise_test = getLaplaceNoiseRange(sensitivity, epsilon, percentile_noise);
	part_A.epsilon_test = epsilon; 
