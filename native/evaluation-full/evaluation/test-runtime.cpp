...
int runtime_testing(int argc, char **argv)
{
	...
	if (argc > 1)
	{
		...
		if (strcmp(argv[9], "1") == 0)
		{
			...
			fout.open("./results/runtime_new_scheme_500K_check_verify_answer.csv", ios::out | ios::trunc);
		...
		{
			fout.open("./results/runtime_new_scheme_500K_check_verify_answer.csv", ios::out | ios::app);
		}
