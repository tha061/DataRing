...
int query_evaluation_with_cheating_detection_random_query_anser_strategy(int argc, char **argv)
{
	...
	float noise_budget_server = 0.5; // 0.5;
	float noise_budget_party = 0.5; //0.5;
	...
	double num_test_each = (double)num_test/3; // 3 test function types
	int num_test_each_type_rounded = floor(num_test_each);
	...
	double tmp1 = getLaplaceNoiseRange(sensitivity, epsilon, percentile_noise); //fixed by Tham Nov 20
     servers.maxNoise = (int) tmp1;
     servers.minNoise = -servers.maxNoise;

  cout<<"Servers: maxNoise = "<< servers.maxNoise<<endl;
	...
	ENC_Stack pre_enc_stack(4*dataset_size, servers.coll_key);
	...
	double tmp = getLaplaceNoiseRange(sensitivity, epsilon, percentile_noise); //fixed by Tham
	part_A.maxNoise = (int) tmp;
	...
	// float amt_lie = 0.1; // 10% of the dataset is fake
	...
	//only gen the real query at once
    ...
    while (itr_query < total_queries)
    {
        ...
        if (query_random == 1) //actual query
        {
			...
			trackTaskStatus(time_track_list, "Query/Test", query_random); 
            trackTaskStatus(time_track_list, "Dataset true/false", answer_strategy);
            bool dummy = 1;
		    trackTaskStatus(time_track_list, "Test result", dummy);
        ...
        else // test function
        {
            if(itr_test_L <= num_test_each_type_rounded)
            {
                ...
                if (answer_strategy == 0)
                {
                    ...
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_L, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
                    count_lied_ans++;
                ...
                {
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_L, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
                ...
                {
                    no_lied_detected++;
					// cout<<"Test L detected where answer is"<<answer_strategy<<endl;
                }
                trackTaskStatus(time_track_list, "Test result", test_status);
            ...
            else if (itr_test_V <= num_test_each_type_rounded)
            {
                ...
                if (answer_strategy == 0)
                {
                    ...
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_V, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
                    count_lied_ans++;
                ...
                {
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_V, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
                ...
                {
                    no_lied_detected++;
					// cout<<"Test V detected where answer is"<< answer_strategy<<endl;
                }
            ...
            else // if (itr_test_n <= num_test_each_type_rounded)
            {
                ...
                {
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_N, sum_cipher, part_A.fake_histogram, servers.coll_key, epsilon);
                    count_lied_ans++;
                }
                else
                {
                    // part_A.computeAnswer_opt(server1.enc_question_map, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
					part_A.computeAnswer_opt(server1.enc_question_map_test_N, sum_cipher, part_A.histogram, servers.coll_key, epsilon);
                }
                ...
                {
                    no_lied_detected++;
					// cout<<"Test n detected where answer is"<<answer_strategy<<endl;
                }
	...
	if (argc > 1)
	{
		...
		string filename = "./results/epsilon_05_cheating_omega_05/epsilon_05_detected_rate_omega_05_";
	...
	// 		string filename = "./results/cheating_5pc_new2/detected_rate_lie_5pc_";
